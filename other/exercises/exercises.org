...
#+BEGIN_SRC python :results output :tangle tree.py
  ...
  class binary_tree(tree):
      ...
      def __init__(self, rootval, children=[]):
          if isinstance(rootval, list):
              self.value = rootval[0]
              self.children = [binary_tree(x) for x in rootval[1:]] + [None for x in range(2-len(rootval[1:]))]
          else:
              self.value = rootval
      ...
      def rec_inord_preord(inorder, preorder):
          ...
          root = binary_tree(preorder[0],
                             [binary_tree.rec_inord_preord(inorder[:rootindex], preorder[1:1+rootindex]),
                              binary_tree.rec_inord_preord(inorder[rootindex+1:], preorder[rootindex+1:])])
          return root

  testtree = binary_tree([3, [1, [2, [3, [4]]]], [3, [2], [3]]])
  print(testtree.get_left())
