...
#+BEGIN_SRC C++ :flags -std=c++14
  ...
  vector<Type> sorted_unique_elements(vector<Type> input) {
       set<Type> set(input.begin(), input.end());
       vector<Type> output(set.begin(), set.end());
       return(output);
  }

  template <typename Type>
  vector <Type> inorder_unique_elements(vector<Type> input) {
  ...
  int main() {
       vector<float> input {1, 9, 2, 3, 4, 6, 7, 7, 8, 9, 0, 0};
       vector<float> sorted_output = sorted_unique_elements(input);
       vector<float> ordered_output = inorder_unique_elements(input);
       for (float i : input) {
            cout << i << " ";
       }
       cout << endl;
       for (float i : sorted_output) {
            cout << i << " ";
       }
       cout << endl;
       for (float i : ordered_output) {
            cout << i << " ";
       }
  }
